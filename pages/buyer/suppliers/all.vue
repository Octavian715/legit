                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-800 text-subtitle3 mt-1">
                            {{
                                totalSuppliers > 0
                                    ? t('allSuppliersPage.suppliersCount', {
                                          count: totalSuppliers,
                                      })
                                    : isLoading
                                      ? t('allSuppliersPage.loadingSuppliers')
                                      : t('allSuppliersPage.noSuppliers')
                            }}
                        </p>
                    </div>
...
<script setup lang="ts">
    ...
    definePageMeta({ middleware: ['buyer'], layout: 'app' })
    ...
    const totalSuppliers = computed(() => userDashboardStore.totalSuppliers || 0)
    const totalItems = computed(() => userDashboardStore.suppliersMeta?.total || 0)
    ...
    const suppliers = computed(() => userDashboardStore.suppliers)
    const suppliersMeta = computed(() => userDashboardStore.suppliersMeta)
    ...
    const breadcrumbs = computed(() => [
        ...
        { label: t('navigation.allSuppliers'), to: localePath('/buyer/suppliers/all') },
    ])
    ...
    const fetchSuppliers = async () => {
        ...
        try {
            ...
            const success = await userDashboardStore.fetchAllSuppliers(filters)

            if (requestId !== currentRequestId) {
                return false
            }

            // if (success && suppliersMeta.value?.total !== undefined) {
            //     const newTotalPages = Math.ceil(suppliersMeta.value.total / itemsPerPage.value) || 1
            ...
            toast.error(error.message || t('userDashboard.errors.fetchSuppliersFailed'))
            return false
        } finally {
            if (requestId === currentRequestId) {
    ...
    const debouncedFetch = useDebounceFn(fetchSuppliers, 300)
