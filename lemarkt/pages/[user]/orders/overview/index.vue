    ...
    watch(
        ...
        async () => {
            if (hasLoadedOnce.value) {
                await nextTick()
                setTimeout(() => {
                    if (buyerSpentCategoryChartRef.value?.updateChart) {
                        buyerSpentCategoryChartRef.value.updateChart()
                    }
                }, 100)
            }
        },
    ...
    watch(
        ...
        async () => {
            if (hasLoadedOnce.value) {
                await nextTick()
                setTimeout(() => {
                    if (buyerSpentSupplierChartRef.value?.updateChart) {
                        buyerSpentSupplierChartRef.value.updateChart()
                    }
                }, 100)
            }
        },
    ...
    watch(
        ...
        async () => {
            if (hasLoadedOnce.value) {
                await nextTick()
                setTimeout(() => {
                    if (buyerOrdersChartRef.value?.updateChart) {
                        buyerOrdersChartRef.value.updateChart()
                    }
                }, 100)
            }
        },
