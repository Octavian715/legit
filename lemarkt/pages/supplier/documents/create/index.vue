            ...
            <div v-if="error" class="mb-6 p-4 bg-red-50 border border-red-200 rounded">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-subtitle3 text-red-800 mt-1">
                            {{ error }}
                        </p>
                    </div>
                    <button
...
<script setup lang="ts">
    ...
    const pageTitle = computed(() => {
        return !isFormVisible.value
            ...
            : t('addATemplate', { template: t('orders.documents', { n: 0 }) })
    })
    const breadcrumbs = computed(() => [
        { label: t('home'), to: localePath('/supplier/dashboard') },
        { label: t('navigation.documents'), to: localePath('/supplier/documents/all') },
        { label: t('navigation.createDocument'), to: localePath('/supplier/documents/create') },
    ])
    ...
    const handleSuccess = async (document?: any) => {
        ...
        toast.success(t('orders.success.created'), t('orders.success.createdTitle'))
