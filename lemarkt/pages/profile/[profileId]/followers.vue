...
<script setup lang="ts">
    ...
    // const updateFollowStatusLocally = (followerId: number, isFollowing: boolean): void => {
    //     if (!Array.isArray(followers.value)) return
    //     const followerIndex = followers.value.findIndex((f) => f.id === followerId)
    //     if (followerIndex === -1) return
    //     // Update is_following status in user object
    //     if (followers.value[followerIndex].user) {
    //         followers.value[followerIndex].user.is_following = isFollowing
    //     }
    // }
    ...
    const handleConnectToggle = async (
        companyId: number,
        action: 'request' | 'cancel' | 'disconnect',
        data: any = {}
    ) => {
        console.log(companyId, action, data)
        const company = companies.value.find((item) => item.id === companyId)
