...
<script setup lang="ts">
    ...
    const businessTypeOptions = computed(() => {
        return (
            networkStore.followerFilters?.business_types?.filter(
    ...
    const languageOptions = computed(() => {
        return (
            networkStore.followerFilters?.spoken_languages?.filter(
                (item) => item && typeof item.id === 'number'
            ) || []
        )
    })
    ...
    const countryOptions = computed(() => {
        return (
            networkStore.followerFilters?.countries?.filter(
                (item) => item && typeof item.id === 'number'
            ) || []
        )
    })
    ...
    const initializeFilterData = async (): Promise<void> => {
        ...
        try {
            ...
            console.error('Failed to initialize follower filter data:', error)
            initializationError.value = error.message || t('network.followers.filters.loadError')
        } finally {
    ...
    const syncFiltersFromProps = () => {
        if (props.filters) {
            ...
            selectedBusinessTypeIds.value = [...(props.filters.businessTypeIds || [])]
            selectedLanguageIds.value = [...(props.filters.spokenLanguageIds || [])]
            selectedCountryIds.value = [...(props.filters.countryIds || [])]

            if (props.filters.dateRange) {
                dateRangeModel.value = {
                    start: props.filters.dateRange.startDate,
                    end: props.filters.dateRange.endDate,
                }
            } else {
                dateRangeModel.value = null
            }
